	...
	: socketName_(name), socketClient_({ 0 }), socketServer_({ 0 }), clientInitialized_(false)
{
	...
	ret = nng_rep0_open(&socketServer_);
	if (ret != 0)
	{
		std::cout << "[Fatal Error] nng_rep0_open: " << nng_strerror(ret);
		std::exit(1);
	}

	ret = nng_listen(socketServer_, (std::string("ipc:///tmp/") + name + "_server.ipc").c_str(), NULL, 0);
	if (ret != 0)
	{
		std::cout << "[Fatal Error] nng_listen: " << nng_strerror(ret);
		std::exit(1);
	}
}
...
void MessageSocket::CreateClient()
{
	...
	ret = nng_req0_open(&socketClient_);
	if (ret != 0)
	{
		std::cout << "[MessageSocket::CreateClient Fatal Error] nng_req0_open: " << nng_strerror(ret);
		std::exit(1);
	}

	ret = nng_dial(socketClient_, (std::string("ipc:///tmp/") + socketName_ + "_client.ipc").c_str(), NULL, 0);
	if (ret != 0)
	{
		std::cout << "[MessageSocket::CreateClient Fatal Error] nng_dial: " << nng_strerror(ret);
		std::exit(1);
	}
...
bool MessageSocket::ReceiveRequest(std::string& message)
{
	...
	ret = nng_recv(socketServer_, &buf, &sz, NNG_FLAG_ALLOC | NNG_FLAG_NONBLOCK);
	if (ret == NNG_EAGAIN)
	{
		return false;
	}
	...
	if (ret != 0)
	{
		std::cout << "[Fatal Error] nng_recv: " << nng_strerror(ret);
		std::exit(1);
		return false;
	}
...
void MessageSocket::SendResponse(const std::string& message)
{
	...
	ret = nng_send(socketServer_, static_cast<void*>(const_cast<char*>(message.data())), message.size(), 0);
	if (ret != 0)
	{
		std::cout << "[Fatal Error] nng_send: " << nng_strerror(ret);
		//std::exit(1);
	}
}
...
bool MessageSocket::ReceiveResponse(std::string& message)
{
	...
	ret = nng_recv(socketClient_, &buf, &sz, NNG_FLAG_ALLOC | NNG_FLAG_NONBLOCK);
	if (ret == NNG_EAGAIN)
	{
		return false;
	}
...
void MessageSocket::SendRequest(const std::string& message)
{
	...
	ret = nng_send(socketClient_, static_cast<void*>(const_cast<char*>(message.data())), message.size(), 0);
	if (ret != 0)
	{
		std::cout << "[Fatal Error] nng_send: " << nng_strerror(ret);
		//std::exit(1);
